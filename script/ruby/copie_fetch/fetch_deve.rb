...
# Percorso dinamico verso Rails.root/public/download
FILEPATH = File.expand_path('../../public/download/', __dir__) + '/'
...
class SnaiFetcher
  ...
  def initialize(code:)
    ...
    cred_path = File.join(base_dir, code, "credentials")
    cert_path = File.join(base_dir, code, "cert.pem")
    key_path  = File.join(base_dir, code, "key.pem")

    unless File.exist?(cred_path)
      raise "File credentials non trovato: #{cred_path}"
    end

...
def clean_csv(csv_data)
  ...
  # Split in righe e ripulisce
  lines = csv_data.lines.map.with_index do |line, idx|
    line = line.strip
    next if line.empty?
    if idx == 0
      # prima riga: rimuove qualsiasi " o ( iniziale
      line.sub!(/\A[\("]+/, '')
    else
      # righe dati: rimuove n" iniziale
      line.sub!(/\An"/, '')
    end
    # Rimuove TUTTI i doppi apici dalla riga
    line.gsub!('"', '')
    line
  end

  # Rimuove eventuali righe nil (vuote) e join senza interlinee
  lines.compact.join("\n")
end
